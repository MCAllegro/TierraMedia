...
public class UserDAOImpl implements UserDAO {
	...
	public int updateCompra(User user) {
		try {
	...
	public int erase(Integer id) {
		try {
			String sql = "UPDATE USERS SET ERASE=1 WHERE ID = ?";
			Connection conn = ConnectionProvider.getConnection();
	...
	public User findByUsername(String username) {
		try {
			String sql = "SELECT * FROM USERS WHERE USERNAME = ? AND ERASE = 0";
			Connection conn = ConnectionProvider.getConnection();
			PreparedStatement statement = conn.prepareStatement(sql);
	...
	public User find(Integer id) {
		try {
			String sql = "SELECT * FROM USERS WHERE ID = ? AND ERASE = 0";
			Connection conn = ConnectionProvider.getConnection();
			PreparedStatement statement = conn.prepareStatement(sql);
			statement.setInt(1, id);
			ResultSet resultados = statement.executeQuery();
	...
	public List<User> findAll() {
		try {
			String sql = "SELECT * FROM USERS WHERE ERASE = 0";
			Connection conn = ConnectionProvider.getConnection();
			PreparedStatement statement = conn.prepareStatement(sql);
			ResultSet resultados = statement.executeQuery();
