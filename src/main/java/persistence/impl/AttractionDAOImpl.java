...
public class AttractionDAOImpl implements AttractionDAO {
	...
	public List<Attraction> findAll() {
		try {
			String sql = "SELECT * FROM ATTRACTIONS WHERE ERASE = 0";
	...
	public Attraction find(Integer id) {
		try {
			String sql = "SELECT * FROM ATTRACTIONS WHERE id = ? AND ERASE = 0";
			Connection conn = ConnectionProvider.getConnection();
			PreparedStatement statement = conn.prepareStatement(sql);
	...
	public int erase(Integer id) {
		try {
			String sql = "UPDATE ATTRACTIONS SET ERASE=1 WHERE ID = ?";
			Connection conn = ConnectionProvider.getConnection();
	...
	public int updateCompra(Attraction atraccion) {
		try {
		String sql = "UPDATE ATTRACTIONS SET CAPACITY=? WHERE ATTRACTIONS.id = ?;";
		
			Connection conn = ConnectionProvider.getConnection();
			PreparedStatement statement = conn.prepareStatement(sql);
			int i = 1;
			statement.setInt(1, atraccion.getCapacity());
