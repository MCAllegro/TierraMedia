...
<link rel="stylesheet" href="assets/stylesheets/styles.css">
    <title>Turismo tierra media - Promociones</title>
</head>
    ...
    <header class="container-fluid bg-secondary bg-gradient text-white pt-1">
        ...
        <nav class="bg-transparent text-white container-fluid mt-3">
            <div class="row d-flex justify-content-between text-center">
                ...
                <div class="col-2">
                    <a class="nav-link border border-dark border-3 bg-dark text-white rounded" aria-current="page" href="./index.do">Promociones</a>
                </div>
                <div class="col-2">
                    <a class="nav-link border border-dark border-3 bg-dark text-white rounded" aria-current="page" href="../itinerario/index.do">Itinerario</a>
                </div>
            </div>
        </nav>
    </header>
        ...
        <article class="container mt-4 text-center">
            ...
            <c:if test="${success != null}">
                ...
                <table class="table table-responsive">
                    ...
                    <tr>
                    	<td><c:out value="${promos.name}"></c:out></td>
                    	<td><c:forEach items="${promos.getListaInterna()}" var="atracs"> <c:out value="${atracs.name}"></c:out>,</c:forEach></td>
                    	<td><c:out value="${promos.cost}"></c:out></td>
                    	<td><c:out value="${promos.duration}"></c:out>Hs</td>
                    	<td>
                    	<c:choose>
								<c:when
									test="${user.canAffordPromo(promos) && user.canAttendPromo(promos) && promos.canHost()}">
									<a href="/TierraMedia/promos/buy.do?id=${promos.id}"
